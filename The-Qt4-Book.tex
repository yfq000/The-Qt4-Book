...
\end{document}
    ...
    图3.1 Spreadsheet应用程序
    在绝大多数图形用户界面应用程序的后台，都有一套提供底层功能的代码——例如，用于读
写文件或者用于处理用户界面中的数据的代码。在第4章，将会再次把Spreadsheet应用程序当作
实例，看看如何实现这些功能。
3.1  子类化QMainWindow
    通过子类化QMainWindow，可以创建一个应用程序的主窗口。由于QDialog和QMainWindow都
派生自QWidget，所以在第2章中看到的许多创建对话框的技术，对于创建主窗口也同样适用。
    可以使用Qt设计师创建应用程序的主窗口，但是在这一章，将使用代码来完成所有的功能，
以便可以说明它们是如何完成的。如果你更喜欢可视化的方式，可以参考Qt设计师在线手册中
的“Creating a Main Window Application”一章。
    Spreadsheet应用程序主窗口的源代码分别放在maimvindow.h和mainwindow. cpp中。先从头文
件开始分析：
36    C++ GUI Qt 4编程（第二版）
群ifndef MAINWINDOW_H
群define MAINWINDOW H
#include <QMainWindow>
class
class
class
...
protected:
   void closeEvent(QCloseEvent *event);
    、我们将MainWindow类定义为QMainWindow类的子类。由于类MainWindow提供了自己的信号
和槽i所以它包含了Q_OBJECT宏。
    closeEvent()函数是.QWidget类中的一个虚函数，当用户关闭窗口时，这个函数会被自动调用。
类MzunWindow中重新实现了它，这样就可以向用户询问一个标准问题“Do you want to save your
changes?”，并且可以把用户的一些偏好设置保存到磁盘中。
p rivate  slots :
        void  newFile( ) ;
       void open () ;
        bool  save ( ) ;
 ...
 ' \/oid a'bout(};
    +  ：
    像File-*New和Help-*About这样的菜单项，在MainWindow中会被实现为私有槽。除了save()
槽和saveAs()槽返回一个bool值以外，绝大多数的槽都把void作为它们的返回值。当槽作为一个
信号的响应函数而被执行时，就会忽略这个返回值；但是当把槽作为函数来调用时，其返回值对我
们的作用就和调用任何一个普通的C++函数时的作用是相同的。
void openRecentFile() ;    .
...
private:
   ...
   /  ’void writeSetting,i() ;  .
    bool okToContinue();        ,             .    ' t
  ' bool410adFile(const QString;&fileName);  ,
    bool saveFile(const QString &fileName);
 .' .  void' setCurrentFile(-const QStririg &fikName) ;
 .      void  updateRecentFileActions ( ) ;
         ost ring  st rippedName ( const  QSt ring &fullFileName) ;
力了能够对用户界面提供支持，主窗口需要更多的私有槽以及数个私有函数。
f
＼鞋
...
#endif
    除了它自己的私有槽和私有函数以外，MainWindow类还有很多私有变量。
    _    ‘
和私有函数时，将再对它们进行解释。
现在来看看实现文件：
#include <QtGui>
...
"spreadsheet.h"
驾用到这些私有槽
我们包含了<QtGui>头文件，其中包含了在子类中所要用到的所有Qt类的定义。我们也包含
了一些自定义头文件，特别是来自第2章的finddialog.h、
...
readSettings-( ) ;
findDialog = o;
setWindowlcon (QIcon ( " :/images/icon . png " 》 ;
setCurrentFile( " ") ;
dialog.h和sortdialog.h三个头文件。
    一    ，：一
    在这个构造函数中，先从创建一个Spreadsheet窗口部件并且把它设置为这个主窗口的中央窗
口部件开始。中央窗口部件会占用主窗日的中央区域部分（如图3.2所示）。Spreadsheet类是
QTableWidget类的一个子类，并且也具有一些电子制表软件的功能，如对电子制表软件公式的支持
等。将会在第4章中实现这一功能。    ~
    私有函数createAct/ons()、createMenus()、createContexdVIenu()、createToolBars()和createStatusBar()创
建主窗口中的其余部分，readSettings()则读取这个应用程序存储的一些设置。
。＼i飞3，
C++ GUI Qt4编程（第二版）
图3.2 QMainWindow中的区域分配
    我们把findDialog指针初始化为空(nuⅡ)指针。在第一次调用MainWindow:：find()函数时，将会
创建该FindDialog对象。
    在构造函数的最后部分，把窗口的图标设置为icon. png，它是一个PNG格式的文件。Qt支持
很多图像格式，包括BMP、GIF、JPEG、PNG、PNM、SVG、TIFF、XBM和XPM。调用QWidget：：setWindow-
Icon()函数可以设置显示在窗口左上角的图标。遗憾的是，还没有一种与平台无关的可在桌面上
显示应用程序图标的设置方法。与平台相关的桌面图标设置方法在http：//doc.trolltech.com/4.3/
appicon. html中进行了阐述。
...
文件。）
o使用Qt的资源机制(resource mechanism)。
    达里，使用了Qt的资源机制法，因为它比运行时载人文件的方法更方便，并且该方法适用于
所支持的任意文件格式。我们将选中的图片存放在源代码树中名为images的子目录下。
    为了利用Qt的资源系统(resource system)，必须创建一个资源文件(resource file)，并且在识别该
资源文件的．pro文件中添加一行代码。在这个例子中，已经将资源文件命名为spreadsheet. qrc，因
此只需在．pro文件中添加如下一行代码：
    资源文件自身使用了一种简单的XML文件格式。这里给出的是从已经使用的资源文件中摘
录的部分内容：
<file>images/icon.png</file>
               <file>iWiages/goto cell . png</file>
<／qresource>
<／RCC>
第3章创建主窗口
    所有资源文件都会被编译到应用程序的可执行文件中，因此并不会弄丢它们。当引用这些资
源时，需要使用带路径前缀：，（冒号斜线）的形式，这就是为什各会将图标表示成：/images/icon.png
    J    '
的形式。资源可以是任意类型的文件（并非只是一些图像），并且可以在Qt需要文件名的大多数
地方使用它们。第12章将对此做进一步说明。    ’’
3.2创建菜单和工具栏
    绝大多数现代图形用户界面应用程序都会提供一些菜单、上下文菜单和工具栏。菜单可以让
用户浏览应用程序并且可以学会如何处理一些新的事情，上下文菜单和工具栏则提供了对那些经
常使用的功能进行快速访问的方法。图3.3展示的是Spreadsheet应用程序的菜单。

图3.3 Spreadsheet应用程序中的菜单
    Qt通过“动作”的概念简化了有关菜单和工具栏的编程。一个动作(action)就是一个可以添加
  到任意数量的菜单和工具栏上的项。在Qt中，创建菜单和工具栏包括以下这些步骤：
    o创建并且设置动作。
    ●创建菜单并且把动作添加到菜单上o    j    ：+。
．    o创建工具栏并且挹动作添加到工具栏上。
    在这个Spreadsheet应用程序中，动作是在createActions()函数中创建的：
    动作New有一个加速键(New)、一个父对象（主窗口）、一个图标、一个快捷键和一个状态提示。
  大多数窗口系统都有用于特定动作的标准化的键盘快捷键。例如，在Windows、KDE和CNOME中，
  这个New动作就有一个快捷键C仕l+N，而在Mac oS X中则是Co眦蛐d+N。通过使用适当的
  QKeySequence：：S协dard】细枚举值，就可以确保Qt能够为应用程序在其运行的平台上提供正确的
  快捷键。    ，／
    把这食动焦勘迫瓷葛秦S篡耋连接到主窗口的私有堕竺篓烈丞三：-将会在下一节实现它。这
  个连接可以确保在用户：≥媳丽砑强弱敷麓雳茳其翟上的New按钮或者按下ctrl+N时，都
  可以调用newF1丑e()槽。
一．    由于菜单中的0pn、Save和Save As动作与№w动作非常相似，所以将会直接跳到瑚e菜单中
  的“recendy openedlles”（最近打开的文件）的部分。

    我们为recentFileActions数组添加动作。，每个动作都是隐式的，并且会被连接到
File()槽。稍后，将会看到如何读这些最新文件中的动作变得可见并且可用。


Grid动作和Auto．Recalculate动作是相互独立的两个复选动作。通过QActionGroup类的支
持，Qt也可以支持相互排斥的动作。    ，，．

Qt动作，通过访问qApp全局变量，我们可以使用QApplication对象的aboutQt()槽。
这个动作会弹出一个如图3.4所示的对话框。  ’    ，  ．    ．．

图3:4  About Qt对话框
现在已经创建了这些动作，还可以继续构建一个包含这些动作的菜单系统0
．在Qt中，菜单都是QMenu的实例。addMenu()函数可以用给定的文本创建一个QMenu窗口部
    ’‘    一  +
件，并且会把它添酾蓟菜覃翟牵。QMainWindow：：menuBar()函数返回一个指向，QMenuBar的指针。
出来：‘    。  1
    ，    ‘．，、：    ‘
和Save As动作添加进去。插入一个间隔器
(separator)，可以从视觉上把关系密切的这些项放在一起。使用二个for循环从recentFiieAcUons数
组中添加一些动作（最初是隐藏起来的），然后在最后添加一个exitAct/on动作。    ；
。  我们已经让一个指针指向了这些间隔器中的某一个。这样就可以允许隐藏（如果设有最近文
件的话）或者显示那个间隔器，因为不希望出现在两个间隔器之间什么都没有的情况。
  。现在来创建Edit菜单，就像在File菜单中所做的那样使用QMenu:：addMenu()函数添加各个动
、  作，并且在惹望出现子菜单的地方使用．Menu:：addMemr()函数添加子菜单。一个子菜单与它所


    通过类似的方式创建Tools、Options和Help菜单。在Options菜单和Help菜单之间插入一个间
隔器。对于Motif和CDE风格，这个间隔器会把Help菜单放到菜单栏的最右端；对于其他的风格，
则特会忽略这个间隔器。图3.5是这两种情况的示意图。

．ext葡商q避巧了设置为二个显示这些动作的上下文菜单。‘当用户在一
图3:6 Spreadsheet应用程个窗口部件上单击鼠标右键，或者是在键盘上按下一个与平台相关的
    序中的上下文莱单按键时，就可以激活这些上下文菜单。Spreadsheet中的上下文菜单如
    图3.6所示。
    ‘种更为高级的提供上下文菜单方法是重新实
QMenu窗口部件，在其中添加所期望的那些动作，并

创建工具栏与创建菜单的过程很相似，我们据此创建一个File工具栏和一个Edit工具栏。就
像菜单一样，工具栏也可以有多个间隔器，如图3.7所示。
蒸■蠢羹
3.3设置状态栏
图3.7 Spreadsheet应用程序的工具栏

    随着菜单和工具栏的完成，已经为设置Spreadsheet应用程序的状态栏做好了准备。在程序的
普通模式下，状态栏包括两个状态指示器：当前单元格的位置和当前单元格中的公式。状态栏也
用于显示状态提示和其他一些临时消息。图3.8给出了各种情况下的状态栏。
常规
    状态提示
    临时消息    。
OMainWind刚：：。tat衄ar()函数返回一个指向状态栏的指针。[在第一次调用statusBar()函数
  的时候会创建状态栏。]状态栏指示器是一些笸单的Q垫b哄』亘以在任何需要瞧盟候旦鍪霹它们且氢三殳
  本。已经在fon—1ulaLabel史鋈塑工=仝鱼童磅苎土鎏便让那些在它里面显示的文本能够与它的左
  侧边看二不习滴碥衫匿：当把这些QLabe1添加到状态栏的时候，它们会自动被重定义父对象一，一丛
  便让它们成为状态栏的手对象。    ．
    图3.8所宗萌两个标签都有不同的空间需求。单元格定位指示器只需要非常小的空间，并且
  在重新定义窗口大小时．任何多余的空间都会分配给位于右侧的单元格公式指示器。这是通过在
  公式标签的oStatusBar：：add珊dget()调用中指定一个伸展因子1而实现的。位置指示器的默认伸
i，展因子为0，这也就意味着它不喜欢被伸展。    ．

当QStatusBar摆放这些指示器窗口部件时，它会尽量考虑由QWidget：：sizeHint()提供的每一个
窗口部件的理想大小，然后再对那些可伸展的任意窗．口部件进行伸展以填满全部可用空间。一个
窗口部件的理想大小取决于这个窗口部件的内容以及改变内容时的变化大小。为了避免对定位
指示器连续不断地重定义大小，设置它的最小尺寸大小为它所能包含的最大字符数(“W999”)和一
些空格的总大小。还把它的对齐方式设置为Qt：：AlignHCenter，以便可以在水平方向上居中对齐它
的文本。
    在函数结尾的附近，把Spreadsheet':.的两个信号和MainWindow的两个槽，upda
    upd批s诅t璐Bar()槽可以罂堑差垂揸塞垄遵强强题韭包型每公誊塑示器。只要用户把单元格光
标移动到一个新的单元格，这个槽就会得到调用。该槽也可以作为一个普通函数而在cre批s诅‘us—
Bar()的最后用于初始化这些指示器。因为印论adsheet。不会在一开始的时候就发射c峨n

槽把而hd曲Modified属性设置为姗e，用以更新标题栏。这个函数也会更
新位置和公式指示器，以便可以让它们反映事件的当前状态。
3.4实现File菜单
    在这一节中，将实现那些能够让File菜单项正常工作并且能够对最近打开文件进行管理的槽
函数和私有函数。

    当用户点击File+  New菜单项或者单击工具栏上的New按钮时，就会调用newFile()槽。如果
存在还没有被保存的信息，okToCondnue()私有函数就会弹出如图3,9所示的对话框：“Do you、vant
to save your change。？”。如果用户选择Yes或者No(保存文档应该选择Yes)，这个函数会返回true；
如果用户选择Cancel，它就返回fal。e。Spreadsheet：：clear()函数会清空电子制表软件中的全部单元
格和公式j setCurrentFile'()私有函数会更新窗口的标题，。以说明正在编辑的是一个没有标题的文
档，它还会设置curFile私有变量并耳更新最近打开文件的列表。    ‘  ．    ’

return true;
    在okToContinue()函数中，会检测windowModified属性的状态。如果该属性的值是true，就显示
    f    ．    ‘    ．
一个如图3‘：9所示的消息框。这个消息框包含一个Yes按钮、二个No按钮和一个Caricel按钮。
．／
图3.9   "Do ypu want to save your changes?”消息框
显示的文本内容。
    当首次看到warning()函数调用时，可能会觉得它有点复杂，但这种常用语法实际上是相当简
单的：    ，  。    ．
    QMessageBox::warning(parent, t/tLe, message, buttons);
    除了warning()之外，QMessageBox还提供了infoImation()、question()和cnticai()函数j它们每一
个都有自己特定的图标，这些图标如图3.10历示。~    。

    open()槽对File_一Open做出响应。就像newFile() -样，’它首先调用okToContir/ue()函数来处理
任何没有被保存的变化。然后它使用方便的QFileDialog:：getOpenFdeName()静态函数从用户那里
获得一个新的文件名。这个函数会弹出一个文件对话框，让用户选择一个文件，并且返回这个文
件名——或者，如果用户单击了Cancel按钮，则返回一个空字符串。    。
    传递给QFileDialog：：getOpenFileName.()函数的第一个参数是它的父窗口部件j用于财话框和
其他窗口部件的这种父子对象关系意义并不相同。对话框通常都拥有自主权，，但是如果它有父x寸，
C++ GUI.Qt 4编程（第二版）
象，那么在默认情况下，它就会居中放到父对象上。一个子对话框也会共用它的父对象的任务栏。
    第二个参数是这个对话框应当使用的标题。第三个参数告诉它应当从哪一级目录开始，在这
个例子中就是当前目录。
    第四个参数指定了文件过滤器。文件过滤器( filter)由一个描述文本和一个通配符组成。如果
除了要支持Spreadsheet本地文件格式以外，还需要支持采用逗号分隔的数据文件和Lotus l-2-3文

我们让它成为一个独立的函
如果载人成功，会调用setCurrenOile()
    -√———、—————、、______________√__——__、一
会通过一个消息框把遇到的问题通知

    save()槽对File一Save做出响应。如果因为这个文件是之前打开的文件或者它是一个已经保
存过的文件，这样已经有了一个名字，那么save()函数就会用这个名字调用saveFile()函数；否则，
它只是简单地调用saveAs()函数。

调用QFileDialog:：getSaveFileName()函数来从用户那里
得到一个文件名。如果用户单击了Cancel，则返回~alse，这将会使这个结果向上传递给它的调用者
[ save()或者okToContinue()]。
  如果给定的文件已经存在，ge:t{SayeFeN翼丛韭i数
    ＼，1一一一、’———一
但通过给getSaveFileName()函数传递_个QFileDialog:：
这一行为 o .

当用户单击File—Exit或者单啬窗口标题栏中的差塑按钮时，将会调旦QWidget:.！close()擅n、
受该操作的影响。一般情况下，我们会接受这个事件，这会让Qt隐藏该窗口。也可以调用私有函
数writeSetUngs()来保存这个应用程序的当前设置。
    当最后一个窗百美丽硌j匿下厦鬲程序就结束了。如果需要，通过把QApplication的quitOn-
IatWindowClosed属性设置为false，可以禁用这种行为。在这种情况下，该应用程序将会持续保持
运行 ,直到调用  QApplication:  : quit()函数,程序才会结束。

    在。etCurrentFile()中，对保存正在编辑的文件名的curFile私有变量进行了设置。在把这个文
件名显示在标题栏中之前，需要使用strippedName()函数移除文件名中的路径字符，这样可以使文
件名看起来更友好一些。    ：

每个QWidget都有一个windowModified属性，如果该窗．口的文档存在没有保存的变化，则应当
把它设置为kue，否则应当将其设置为false。在Mac OS X下，未保存的文档是通过窗口标题栏上
关闭按钮中的一个点来表示的；在其他平台下，则是通过文件名字后跟一个星号来表示的。Qt会
自动处理这一行为，只要始终让windowModified属性保持为当前最新状态，并且当需要显示星号的
时候，把“[*]”标记放在窗口的标题栏上即可。
...
使用一个Jai;a风格的迭代器，可以移除任何不再存在的文件。一些文件或许已经在前面的会
...
时，就会调用find()槽来弹出Find对话框。这时，就可能出现下列几种情形：
  ...
  但是推迟对话框的创建过程将可哒德堡壁釜囊鬓茎磊蓑篓还d如果从-hr使用到这不对话
    ——————_——_一
谁骊活耐溪示裔厕蔓受匦塑堕型塑剐丝脚焱
  一‘褒詹习爵-赫舐5：；se()和ac曲ateWindow()来确保窗口位于其他寥且之占羞县墨酉见的和激
...
话框和消息框就是模态的。    。
  ...
  ＼／履Ⅱ桌对话框是通过show()调用的，那么它就是非模态对话框[除非此后又调甩：setModal()，芨会一
...
。我们只创建了一个新的-\MainWindow实例。这看起来有些奇怪；因为没有保留指向这个新窗
...
Ma'inWindow::MainWindow()
{
                                                                                ,*
          setAtt ributAQt :/_WA  DekteOnClo:ej
      N盟曼坚班  se); . .
...
Spreadsheet应用程序。第6章将对肋I进行说明。    ’，，≯    √：．    +i
    ．    ’
    图3.17单文档界面和多文档界面    ．

＼  7 I!许多应用程序都会在启动的时候显示一个程序启动画面（splash screen），图3118给出的就是这
I  ，
58    C++ GUI Qt 4编程（第二版）
...
窦现这些缺少的功能。
    ...
    类Spreadsheet派生自QTableWidget，如图4.1所示。    i。
    eView
...
#ifndef SPREADSHEET H
#define SPREADSHEET:H
...
格中，但是这一次要把制表符“\t"当作分隔符。图4.6给出了这一过程中所使用的步骤。
...
元格位置上又调用了cell()，那么该函数将会返回。个空指针。。    。一    ~
 void  Sp readsheet: :selectCurrentRow ( )
...
窗口部件上时显示该信息。
QSt ring  IconEdito rPlugin: :whatsThis ( )  const
{
...
部件。
QWidget   *IconEditorPlugin : : createWidget ( QWidget  *pa rent )
{.
...
qmake构建工具已经构建了一些预定义变量。．$$[QT_INSTALL_PLUGINS]就是它们当中的一
...
个坐标轴上与记号相对应的那些数字。
    ...
    painter->d rawText(x.y,width, height, alignment, text);
萁中，（菇，），，wtath，height）定义了一个矩形，alignmenL是在这个矩形中文本的位置，而text就是要
...
计算出合适的标记符个数，然后把它的numTicks参数设置成该值。因为、a\djllStA:as()需要对变量
